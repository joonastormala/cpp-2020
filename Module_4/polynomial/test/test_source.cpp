...
bool operator==(const Poly& p, const Wrapper& w) {
    ...
    int non_zeros_p = 0;
     for(auto& i : p) {
        ...
        if(i.second != 0) {
            non_zeros_p++;
            if(index >= w.vector_.size() || w.vector_[index] != i.second)
                return false;
        }
    }
    return non_zeros == non_zeros_p;
...
Poly TAdd2(const std::vector<int>& v1, const std::vector<int>& v2) {
    return MakePoly(v1) += MakePoly(v2);
}   
...
TEST(Poly, add2) {
    ...
    seq2.Reset();
    {
        ...
        gcheck::SequenceArgument inputs_not_reserved0({std::vector<int>({}),std::vector<int>({1}),std::vector<int>({1, 2}),std::vector<int>({5, 3, 3, -2}),std::vector<int>({0, 5, -10, 15, 20})});
        gcheck::SequenceArgument inputs_not_reserved1({std::vector<int>({1, -4}),std::vector<int>({0, 5, -2}),std::vector<int>({1, 2}),std::vector<int>({1, -3, 3, -2}),std::vector<int>({4, 5, -1, 5})});
        TestCase(seq.GetSize(),correct_not_reserved,TAdd2,inputs_not_reserved0,inputs_not_reserved1);
    };
}
...
Poly TSubstract2(const std::vector<int>& v1, const std::vector<int>& v2) {
    return MakePoly(v1) -= MakePoly(v2);
}   
...
TEST(Poly, substract2) {
    ...
    seq2.Reset();
    {
        ...
        gcheck::SequenceArgument inputs_not_reserved0({std::vector<int>({}),std::vector<int>({1}),std::vector<int>({1, 2}),std::vector<int>({5, 3, 3, -2}),std::vector<int>({0, 5, -10, 15, 20})});
        gcheck::SequenceArgument inputs_not_reserved1({std::vector<int>({1, -4}),std::vector<int>({0, 5, -2}),std::vector<int>({1, 2}),std::vector<int>({1, -3, 3, -2}),std::vector<int>({4, 5, -1, 5})});
        TestCase(seq.GetSize(),correct_not_reserved,TSubstract2,inputs_not_reserved0,inputs_not_reserved1);
    };
}
